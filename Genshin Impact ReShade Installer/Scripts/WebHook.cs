	...
	internal abstract class WebHook
	{
		...
		public static async void RebootIsScheduled()
		{
			...
			DiscordMessage message = new DiscordMessage(embeds: new[]
			{
				new DiscordMessageEmbed(color: 3065320, author: new DiscordMessageEmbedAuthor($"✨ Reboot was scheduled on {Os.Name} {Os.Build}"),
					footer: new DiscordMessageEmbedFooter($"📅 {DateTime.Now:HH:mm:ss, dd.MM.yyyy} • {Os.TimeZone} • {Os.Region}"))
			});
		...
		public static async Task<bool> SendLogFiles()
		{
			...
			try
			{
				...
				// Files
				string content1 = "File installer.output.log is empty.";
				if (File.Exists(Log.OutputFile)) content1 = File.ReadAllText(Log.OutputFile);
				DiscordFile attachment1 = new DiscordFile("installer.output.log", Encoding.UTF8.GetBytes(content1));

				string content2 = "File installer.error.log is empty.";
				if (File.Exists(Log.ErrorFile)) content2 = File.ReadAllText(Log.ErrorFile);
				DiscordFile attachment2 = new DiscordFile("installer.error.log", Encoding.UTF8.GetBytes(content2));

				string content3 = "File mod_installation.log is empty.";
				if (File.Exists(Log.ModInstFile)) content3 = File.ReadAllText(Log.ModInstFile);
				DiscordFile gitLog = new DiscordFile("mod_installation.log", Encoding.UTF8.GetBytes(content3));
