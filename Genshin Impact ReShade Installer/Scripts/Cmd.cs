            ...
            try
            {
                ...
                if (result.ExitCode != 0)
                {
                    ...
                    if (Regex.Match(stderr, "(?:80073D02)", RegexOptions.IgnoreCase | RegexOptions.Multiline).Success)
                    {
                        ...
                        Console.WriteLine(
                            "     » Something went wrong. I can't install this package!\n" +
                            $"       Reboot your PC or close all opened apps from Microsoft Store.\n\n{stderr}"
                        );

                        Log.ErrorAuditLog(
                            new Exception(
                                $"I can't install Microsoft.VCLibs (attempt {Installation.VcLibsAttemptNumber}).\n\n{stderr}"),
                            true);
                            ...
                            RegexOptions.IgnoreCase | RegexOptions.Multiline).Success)
                    {
                        ...
                        Log.ErrorAuditLog(
                            new Exception(
                                $"Found missing dependency Microsoft.VCLibs (attempt {Installation.VcLibsAttemptNumber}).\n\n{stderr}"),
                            true);
                        ...
                        // Preparing...
                        Console.WriteLine(
                            $"{Installation.ProcessInt++}/20 - Preparing to install Microsoft Visual C++ 2015 UWP Desktop Package (attempt {Installation.VcLibsAttemptNumber}/3)...");
                        ...
                        // Installing...
                        Console.WriteLine(
                            $"{Installation.ProcessInt++}/20 - Installing Microsoft Visual C++ 2015 UWP Desktop Package...");

                        if (!File.Exists(Installation.VcLibsSetup))
                            Log.ErrorAndExit(
                                new Exception($"I can't find a required file.\n{Installation.VcLibsSetup}"),
                                false, false);
