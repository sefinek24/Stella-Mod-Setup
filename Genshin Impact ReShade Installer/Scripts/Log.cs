...
namespace Genshin_Impact_MP_Installer.Scripts
{
	internal abstract class Log
	{
		...
		public static void ErrorString(string msg, bool tryAgain)
		{
			...
			try
			{
				ToastContentBuilder builder = new ToastContentBuilder().AddText("Oh no! Something went wrong 😿").AddText(msg);
				builder.Show();
			}
			catch (Exception e)
			{
		...
		public static void Error(Exception msg, bool tryAgain)
		{
			...
			try
			{
				ToastContentBuilder builder = new ToastContentBuilder().AddText("Oh no! Error occurred 😿").AddText("Go back to the installer.");
				builder.Show();
			}
			catch (Exception e)
			{
		...
		public static async void ErrorAndExit(Exception log, bool hideError, bool reportIssue)
		{
			...
			if (!hideError)
			{
				...
				try
				{
					ToastContentBuilder builder = new ToastContentBuilder().AddText("Failed to install 😿").AddText("🎶 Sad song... Something went wrong...");
					builder.Show();
				}
				catch (Exception e)
				{
				...
				{
					Console.ForegroundColor = ConsoleColor.Yellow;
					Console.WriteLine($"Visit our Discord server for help or try again. Good luck!\n• Discord: {Program.DiscordUrl} [My username: Sefinek#0001]\n• E-mail: contact@sefinek.net\n• Use the available chat on my website.");
			...
			string reportError = Console.ReadLine()?.ToLower();
			switch (reportError)
			{
				case "y":
				case "yes":
