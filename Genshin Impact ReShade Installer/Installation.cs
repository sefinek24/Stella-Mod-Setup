    ...
    internal abstract class Installation
    {
        ...
        public static async Task Start()
        {
            ...
            if (RegionInfo.CurrentRegion.Name == "RU")
                try
                {
                    var player = new SoundPlayer { SoundLocation = @"Data\sound.wav" };
                    player.Play();
                }
                catch (Exception ex)
                {
            ...
            // ----------------------- 3 -----------------------
            Console.WriteLine(
                $"{ProcessInt++}/12 - Installing Microsoft Visual C++ 2015-2022 Redistributable (x64)...");

            if (!File.Exists(Redist64Setup))
                Log.ErrorAndExit(new Exception($"I can't find a required file.\n{Redist64Setup}"), false, false);

            await Cmd.Execute(Redist64Setup,
                $"/install /quiet /norestart /log \"{Log.Folder}\\VC_redist.x64_installation.log\"", null);
            ...
            // ----------------------- 4 -----------------------
            Console.WriteLine(
                $"{ProcessInt++}/12 - Installing Microsoft Visual C++ 2015-2022 Redistributable (x86)...");

            if (!File.Exists(Redist86Setup))
                Log.ErrorAndExit(new Exception($"I can't find a required file.\n{Redist86Setup}"), false, false);

            await Cmd.Execute(Redist86Setup,
                $"/install /quiet /norestart /log \"{Log.Folder}\\VC_redist.x86_installation.log\"", null);
            ...
            // ----------------------- 9 -----------------------
            Console.WriteLine(
                $"{ProcessInt++}/12 - Installing mod and our launcher in {Folder}...");
            ...
            try
            {
                ...
                client.DownloadFile("https://cdn.sefinek.net/resources/genshin-impact-reshade/unlocker-config",
                    fpsUnlockCfgPath);
            ...
            // ----------------------- 11 -----------------------
            Console.Write(
                $"{ProcessInt++}/12 - Downloading files from cdn.sefinek.net and configuring ReShade... ");
            ...
            if (Regex.Match(Program.ShortcutQuestion, "(?:y)", RegexOptions.IgnoreCase | RegexOptions.Singleline)
                ...
                try
                {
                    ...
                    shortcut.TargetPath = $@"{Folder}\Genshin Impact Mod Pack.exe";
                    shortcut.Save();
            ...
            using (var sw = File.CreateText(IsInstalled))
            {
